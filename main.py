...
# --- TES MODULES ---
# Assure-toi que ces fichiers existent bien dans ton dossier
import config
...
class SurveillanceApp:
    def __init__(self, window, window_title):
        ...
        self.window.configure(bg="#e1e1e1")  # Fond gris clair pro
    ...
    def creer_layout_principal(self):
        ...
        # Note: On ne l'affiche pas tout de suite, la fonction changer_mode s'en charge
    ...
    def convertir_pour_tkinter(self, image_opencv, width):
        """Convertit l'image OpenCV pour qu'elle soit lisible par Tkinter"""
        img_small = redimensionner(image_opencv, width=width)

        # Gestion N&B vs Couleur
    ...
    def update(self):
        ...
        if frame is not None:
            ...
            # CAS A : MODE GRILLE (4 petits écrans)
            if self.current_view == "grid":
                w_grid = 380  # Taille des petites fenêtres
            ...
            # CAS B : MODE ZOOM (1 grand écran)
            else:
                w_zoom = 780  # Taille de la grande fenêtre
